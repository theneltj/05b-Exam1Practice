...
def problem2b(rect, n, delta, win):
    ...
    # -------------------------------------------------------------------------
    rect.attach_to(win)
    xleft = rect.get_lower_left_corner().x
    yleft = rect.get_lower_left_corner().y
    xright = rect.get_upper_right_corner().x
    yright = rect.get_upper_right_corner().y
    for k in range(n):
        rectangle = rg.Rectangle(rg.Point(xleft-(delta*k),yleft+(delta*k)),rg.Point(xright+(delta*k),yright-(delta*k)))
